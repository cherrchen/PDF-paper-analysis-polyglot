...
**状态：** M1–M3 完成；M4–M7 基线已落地（含 1 Layout→N Semantic、结构化表格、作者-年引用与 parser ensemble）。M8 批次 A、B 已落地（本地 workspace、本地 Job 编排）。M8 准入：三项 PRD 表面为已实现 + 合成夹具通过；真实 parser 工具输出未验证。初版计划见 [M8 准入与初版开发计划](docs/development/m8.md)。当前进度以[开发路线图](docs/development/roadmap.md)为准。
