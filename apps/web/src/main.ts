...
import { buildPairs, fragmentsForSide, type MappingBundle, type Pair } from "./mapping.js";
...
async function init(): Promise<void> {
  ...
  try {
    ...
    const drawOverlay = (
      ...
      for (const [nodeId, pair] of pairs) {
        for (const fragment of fragmentsForSide(pair, side)) {
          ...
          button.addEventListener("click", () => void activate(nodeId, side, fragment.pageIndex));
    ...
    const counterpartPage = (pair: Pair, origin: Side): number => {
      if (origin === "source") return pair.target.pageIndex;
      return pair.sources[0]?.pageIndex ?? 0;
    };

    const activate = async (nodeId: string, origin: Side, originPage: number): Promise<void> => {
      const pair = pairs.get(nodeId);
      if (!pair) return;
      activeNodeId = nodeId;
      const destination: Side = origin === "source" ? "target" : "source";
      await renderPage(destination, counterpartPage(pair, origin));
      await renderPage(origin, originPage);
