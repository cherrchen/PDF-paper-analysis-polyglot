...
- An Issue is identified by `stable_uuid(layoutDocumentId, "issue", category, message)`: one `(category, message)` pair is one issue, however often it is observed. Unique `IssueStore.issues` ids are a publication precondition — `validate_bundle_references` checks every bundle document's local store for repeated ids, and a hit fails INDEX, so the run publishes nothing
