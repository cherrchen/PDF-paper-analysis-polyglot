...
`pdf_pipeline.pipeline._write_viewer_assets` 是 `viewerDataVersion: 2` 包的唯一生产者（`mapping.json` + `viewer-meta.json`，原子写入）。canonical MappingBundle 字段照常展开（schema 保持 `mapping` 0.1.0，零变更），并附 viewer 私有投影：
...
- 论文尺度 fragment < 10³，网格 O(1) 足够；接口不变的前提下可替换真 R-tree（见 [M6 note](../../.agents/notes/implemented/feature/2026-09-11-m6-bidirectional-reader.md)）。

## 导航行为（6.3/6.4）

`apps/web/src/mapping.ts` 的 `Pair = { sources: Fragment[]; targets: Fragment[] }` 收集双侧全部 fragments（多 fragment 是一等公民）。`pickCounterpart(pair, origin, originRect)` 给对侧 destination：在对侧「不小于 origin 页码」的最小页上取 y 最近者；不存在则取对侧第 0 个。跳转因此只依赖绑定与几何，绝不假设 `source page ≈ target page`（FR-SYNC-004）。

`apps/web/src/main.ts` 的 `activate(nodeId, origin, fragment)`：

1. destination pane 渲染 `pickCounterpart` 所在页；
...
4. origin 侧重绘保留 `aria-pressed` 选中态；active 态跨翻页保留。
...
`apps/web/src/inspector.ts::renderInspector(root, model, nodeId, opts)`。面板含稳定 id（e2e 选择器）：`#inspector-node-id`（完整 SemanticNodeID）、`#inspector-kind`、`#inspector-anchors`（`source p{n} [x,y w×h]` 双侧逐条）、`#inspector-source-text`、`#inspector-translation`（无 entry 显示 `Not translatable`，BIBLIOGRAPHY_ENTRY 按 PRD FR-CITE-004 不进翻译层）、`#inspector-relations`（双向 relation 按钮可跳转）、`#inspector-confidence`（节点 score + reason + target anchor 置信）、`#inspector-provenance`、`#inspector-issues`（有则显示）、`#inspector-terminology`、`#inspector-citations`（六类引用 mark 切片 + targetNodeId 跳转；越界 mark 显示 `mark out of range` 不抛错）。查看原文/译文即双栏并置，无 pane 内容切换。
...
- `POST /api/retranslate` body `{"nodeIds": [...]}`：200 `{"ok": true, "changed": [...]}`；非法 body/未知节点 400；workspace 未初始化 409；`Content-Length` > 4096 字节 413；其余异常 500。lualatex 输出目录共享，`threading.Lock` 串行化重渲染。

`pdf_pipeline.pipeline.rerender_workspace(workspace_dir, viewer_data_dir=…, node_ids=…)`（FR-TRANS-004：零源 PDF 重解析）：载入 workspace 六份 canonical 文档 → 校验 `node_ids ⊆ translation.entries` 键集 → `retranslate_nodes` → compose → LaTeX 投影 + 编译 → `recover_render_anchors` → 复用旧 mapping 的 source 侧绑定重建 MappingBundle → 重写 workspace 三份文档 + viewer 包。注意重新投影意味着整文档 target 重编译（lualatex 秒级~几十秒），这是有意的代价集中。

前端接线：vite dev/preview 把 `/api` proxy 到 `:8000`；`just serve-reader` 一次起 API + dev server（Playwright webServer 用同一命令）。重译成功后：cache-busting 重取 mapping/meta → 销毁并重建 target 的 pdf.js document（版面已变）→ 重建 ReaderModel/索引 → 重画双侧 overlay 与 Inspector，状态行 `Node re-translated · <id 前 8 位>`。
