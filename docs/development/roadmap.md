...
**最后更新：** 2026-09-16
**当前位置：** M7 Parser Ensemble & Quality 基线已落地。M8 准入已收口为分级证据：三项 PRD 表面均为已实现 + 合成夹具通过；真实 MinerU/Docling/GROBID 工具输出未验证，不阻塞初版 A–E，阻塞把内置 registry 的 primary 换成真实工具。字符级 mapping、`source-derived`/`dense-two-column`、Annotation 层、独立 MathML 交付已从延期清单剔除（非 Initial Product）。M8 初版批次 A（本地 workspace 与阶段状态）、批次 B（Job 编排、失败归因、手动重试、worker 并发）、批次 C（阶段缓存键、失效传播、局部重跑、源重绑）与批次 D（specialist 失败隔离：provider 异常降级为 capability 级 fallback + 一条归属该 provider 的 ERROR Issue + `probe.json.degradations`，EVIDENCE 提交为 `degraded` 并在下次运行重跑，零 bundle 才真失败）均已落地，见 [存储](../architecture/storage.md) 与 [批次 D note](../../.agents/notes/implemented/architecture/2026-09-15-m8-batch-d-specialist-isolation.md)；批次 E（旧 workspace 版本策略与 parser 配置入口：可读版本集合 + 明确拒绝、`PAPER_CAPABILITY_REGISTRY` / `--registry` 覆盖、实际 registry 字节入 EVIDENCE/LAYOUT 键）亦已落地，见 [存储](../architecture/storage.md) 与 [批次 E note](../../.agents/notes/implemented/architecture/2026-09-15-m8-batch-e-parser-config-and-workspace-version.md)；批次 F（本地产品端到端验收：路径式导入面板提交 `POST /api/jobs` 且服务端把 `viewerDataDir` 默认成自己的 `--data-dir`、worker 并入 `serve-reader`、`DualPaneReader.load()` 就地换修订、`PAPER_PUBLISH_FAULT` 回滚缝，以及 PRD §40 产品流与「发布失败仍读上一修订」的 e2e 证明）亦已落地，见 [阅读器](../architecture/reader.md) 与 [批次 F note](../../.agents/notes/implemented/architecture/2026-09-16-m8-batch-f-local-e2e.md)。至此 M8 初版批次 A–F 全部落地。计划见 [M8 准入与初版开发计划](m8.md)。详见 [M8 准入收口](../../.agents/notes/implemented/process/2026-09-12-m8-admission-closeout.md)、[PRD 过滤 roadmap 延期项](../../.agents/notes/implemented/process/2026-09-12-prd-filters-roadmap-deferrals.md)、[可选真实 parser 依赖](../../.agents/notes/implemented/architecture/2026-09-12-optional-parser-adapters.md)、[区域级标注真值](../../.agents/notes/implemented/architecture/2026-09-12-region-level-layout-truth.md)、[Figure PDF fragment](../../.agents/notes/implemented/architecture/2026-09-12-figure-pdf-fragment.md) 与 [M7 落地 note](../../.agents/notes/implemented/architecture/2026-09-12-m7-parser-ensemble.md)。

README 状态：工程脚手架 + 核心文档契约 + Walking Skeleton 端到端管线 + Layout Recovery Engine + Semantic Recovery 基线 + Parser Ensemble + M8 批次 A 本地 workspace + M8 批次 B 本地 Job 编排 + M8 批次 C 阶段缓存键与失效传播 + M8 批次 D specialist 失败隔离 + M8 批次 E 旧 workspace 版本策略与 parser 配置入口 + M8 批次 F 本地产品端到端验收。M8 准入分级见 [m8.md](m8.md)。
...
| M8 Productionization | 批次 A、B、C、D、E、F 已落地（初版完成） | 初版：本地 workspace（A，已完成：阶段状态 + 原子提交 + 中断恢复）+ 任务编排（B，已完成：文件式 Job 队列 + `flock` 认领 + 阶段归因失败 + 手动重试 + worker 并发）+ 缓存（C，已完成：阶段缓存键 = producer 版本 + 上游产物哈希 + 阶段配置；registry / parser dump / 翻译配置 / 渲染模板 / schema 与 pipeline 版本变化使消费该输入的阶段失效；`rerun_from` 局部重跑与 `accept_source_change` 源重绑；翻译缓存行带 `cacheVersion`；见 [storage.md](../architecture/storage.md) 与 [api.md](../architecture/api.md)）+ 失败隔离（D，已完成：`collect_bundles` 返回 `ProviderCollection`，provider 异常隔离为 capability 级降级；registry `fallback` 顶替（已收集实例复用、只降一级、不递归），无顶替则用内建基线；Issue severity 恒 ERROR、category 取既有 `IssueCategory`、`fallback` 记顶替者或内建描述；`probe.json.degradations` 形状恒定；EVIDENCE 以 `degraded` 提交（下次重跑、失败不进缓存）而下游继续；所有 routed provider 全失败才让 EVIDENCE 真失败；见 [storage.md](../architecture/storage.md) 与 [批次 D note](../../.agents/notes/implemented/architecture/2026-09-15-m8-batch-d-specialist-isolation.md)）+ 兼容与 parser 接入（E，已完成：可读版本集合 `SUPPORTED_WORKSPACE_VERSIONS` + 集合外明确拒绝且不触碰磁盘，不写版本转换函数；`PAPER_CAPABILITY_REGISTRY` / `--registry` 覆盖入口，实际使用的 registry 字节入 EVIDENCE/LAYOUT 键，坏配置抛 `CapabilityRegistryError` 而不回退；内置 primary 仍是 sim；见 [存储](../architecture/storage.md) 与 [批次 E note](../../.agents/notes/implemented/architecture/2026-09-15-m8-batch-e-parser-config-and-workspace-version.md)）+ 本地产品端到端验收（F，已完成：路径式导入面板 → `POST /api/jobs` 且服务端把 `viewerDataDir` 默认成自身 `--data-dir`、`serve-reader` 含 worker、`DualPaneReader.load()` 就地换修订、`PAPER_PUBLISH_FAULT` 回滚缝、PRD §40 与上一修订存活的 e2e 证明；见 [阅读器](../architecture/reader.md)、[存储](../architecture/storage.md) 与 [批次 F note](../../.agents/notes/implemented/architecture/2026-09-16-m8-batch-f-local-e2e.md)）。Typst、HTML、Analysis Layer 为后续扩展。计划：[m8.md](m8.md) |
...
| 7.7 Regression Benchmark | 基线 | `tests/benchmark/run_benchmark.py` + `tests/benchmark/baseline.json`；缺失 fixture / 可测指标变 null / ERROR+FATAL 增加即非零退出；PR CI Python job 与 `just ci` 在 `just latex-smoke` 之后运行 `just benchmark` |

**M7 Exit Gate：** 基线达成——provider/算法升级现在可以通过 `just benchmark` 与 baseline 对比量化 improved/unchanged/regressed。三项 PRD 表面的准入分级见 [M8 准入与初版开发计划](m8.md)，不以错误的语义准确率名称作为收口证据。决策见 [M7 落地 note](../../.agents/notes/implemented/architecture/2026-09-12-m7-parser-ensemble.md)；延期清单过滤见 [PRD 过滤 roadmap 延期项](../../.agents/notes/implemented/process/2026-09-12-prd-filters-roadmap-deferrals.md)。

### 建议下一步

1. 按 [M8 准入与初版开发计划](m8.md) 的批次 A 开始本地 workspace，不要先做完整 M8，也不要把真实 parser 活服务当成 A–D 的前置。
...
**初版范围与分批计划**（权威当前态）：[`m8.md`](m8.md)。初版只做 8.1–8.6 的本地 workspace、增量、缓存、失败隔离、schema/workspace 兼容与 parser 接入。**8.7 Typst/HTML 与 8.8 Analysis Layer 按 PRD §44 / R7–R8 为后续扩展，不是 M8 初版验收。**
...
**后续扩展（非初版）。** 增加 Typst / HTML Backend 必须复用 SemanticDocument、TranslationLayer、RenderDocument。Initial Product 渲染后端仍是 LaTeX / LuaLaTeX。
...
**后续扩展（非初版）。** 摘要 / RAG / QA 等属于 AnalysisLayer，绝不修改 Source SemanticDocument。
...
初版验收（权威清单）：[`m8.md`](m8.md) 批次 A–F。长期总路线仍包含 Renderer 可扩展；**初版不把 Typst/HTML/Analysis 当 Exit Gate。**
