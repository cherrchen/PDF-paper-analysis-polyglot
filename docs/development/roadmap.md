...
| M8 Productionization | 批次 A、B 已落地，C–F 未开工 | 初版：本地 workspace（A，已完成：阶段状态 + 原子提交 + 中断恢复）+ 任务编排（B，已完成：文件式 Job 队列 + `flock` 认领 + 阶段归因失败 + 手动重试 + worker 并发；见 [storage.md](../architecture/storage.md) 与 [api.md](../architecture/api.md)）/ 缓存（C）/ 失败隔离（D）/ 兼容与 parser 接入（E–F）。Typst、HTML、Analysis Layer 为后续扩展。计划：[m8.md](m8.md) |
