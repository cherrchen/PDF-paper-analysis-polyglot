...
| M8 Productionization | Batches A, B, and C landed, D–F not started | v1: local workspace (A, done: stage state + atomic commit + interrupt resume) + job orchestration (B, done: file-backed job queue + `flock` claims + per-stage failure attribution + manual retry + worker concurrency) + cache (C, done: stage cache key = producer version + upstream artifact hashes + stage config; registry / parser dump / translation config / render template / schema and pipeline version changes invalidate the consuming stages; `rerun_from` local rerun and `accept_source_change` source rebinding; translation cache rows carry `cacheVersion`; see [storage.en.md](../architecture/storage.en.md) and [api.en.md](../architecture/api.en.md)) / failure isolation (D) / compatibility and parser access (E–F). Typst, HTML, and the Analysis Layer are later extensions. Plan: [m8.en.md](m8.en.md) |
