...
| M8 Productionization | Batches A and B landed, C–F not started | v1: local workspace (A, done: stage state + atomic commit + interrupt resume) + job orchestration (B, done: file-backed job queue + `flock` claims + per-stage failure attribution + manual retry + worker concurrency; see [storage.en.md](../architecture/storage.en.md) and [api.en.md](../architecture/api.en.md)) / cache (C) / failure isolation (D) / compatibility and parser access (E–F). Typst, HTML, and the Analysis Layer are later extensions. Plan: [m8.en.md](m8.en.md) |
