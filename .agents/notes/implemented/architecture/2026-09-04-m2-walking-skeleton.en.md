...
6. **LaTeX renderer (2.5)**: `templates/latex/generic-academic.tex` (heading/paragraph/figure/caption) plus the `pdf_pipeline.render_latex` projection. Per [LaTeX as the initial rendering backend](./2026-09-03-latex-as-initial-rendering-backend.en.md): template owns presentation, projection owns structure, TeX never enters SemanticDocument, single backend with no Renderer trait.
