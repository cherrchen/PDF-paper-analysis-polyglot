...
3. **有标签后：** `quality_report` 用 labeled metrics；`paragraphLabelRecall` / `headingLabelRecall` 分别对 `PARAGRAPH_LIKE` / `HEADING_LIKE` 计算布局标签召回（不是段落合并或章节树）。校准样本改为 fused confidence vs IoU 命中。名称更正见 [M8 前审查修复](../bug-fix/2026-09-12-m8-pre-review-repairs.md)。
...
- `tests/benchmark/baseline.json` 因真值粒度从片段改为 IoU+标签而更新：旧 regionRecall=1.0 是 snippet 命中，不是区域精度；新数字更低但是诚实门禁。`paragraphLabelRecall` / `headingLabelRecall` 从 null 变为可测的标签召回。校准仍是 diagnostic。
