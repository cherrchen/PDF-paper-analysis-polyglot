...
- `apps/web`：`index.html` 整文件替换（保留全部既有 id，`#inspector-toggle` 由 button 改为 checkbox），新增 `surface-tabs.ts` / `select-panel.ts` / `history-panel.ts` / `status-panel.ts` / `jobs-client.ts`，`reader.ts` 删 `canvasHeight` 并改用比例滚动，`main.ts` 启动 tabs 与四个面板。控制台的视觉语言与 masthead 主操作此后由 [2026-09-17 Overleaf 风格设计系统 note](./2026-09-17-overleaf-console-design-system.md) 规定，本 note 的决策 1–8（四表面划分、悬浮抽屉、fit-width、upload/select/status 行为）继续有效。
