...
6. **LaTeX renderer（2.5）**：`templates/latex/generic-academic.tex`（heading/paragraph/figure/caption）+ `pdf_pipeline.render_latex` 投影层。遵循 [LaTeX 作为初始渲染后端](./2026-09-03-latex-as-initial-rendering-backend.md)：模板管呈现、投影管结构、TeX 永不进入 SemanticDocument、单一后端无 Renderer trait。
