...
3. **Once labels exist:** `quality_report` uses labeled metrics; `paragraphLabelRecall` / `headingLabelRecall` score `PARAGRAPH_LIKE` / `HEADING_LIKE` layout-label recall (not paragraph merge or the section tree). Calibration samples become fused confidence vs IoU hit. Name correction: [M8-pre review repairs](../bug-fix/2026-09-12-m8-pre-review-repairs.en.md).
...
- `tests/benchmark/baseline.json` is updated because truth granularity changed from snippets to IoU+label: the old regionRecall=1.0 was snippet matching, not region precision; the new numbers are lower but honest. `paragraphLabelRecall` / `headingLabelRecall` become measurable label recall instead of null. Calibration stays diagnostic.
