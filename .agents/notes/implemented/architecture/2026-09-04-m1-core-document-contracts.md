...
1. **六套 canonical JSON Schema**（`schemas/<name>/schema.json`，版本 `0.1.0`）：
   - `common` — ID 类型（ULID/UUID pattern）、Provenance、Origin、Resource、Issue、Geometry（Rect/Quad/Polygon）、Matrix、LayoutLabel
   - `physical-document` — 页、TextSpan、ImageObject、VectorObject、LinkObject、Canonical Page Space
   - `evidence` — RegionCandidate、TableCandidate、FormulaCandidate、MetadataCandidate；provider 字段禁止 extra keys，原生标签保留在 `providerLabel`
   - `layout-document` — LayoutRegion、PageBand、Column、ReadingFlowGraph（graph 为 source of truth，`primaryFlow` 为派生）、ReadingOrderReason
   - `semantic-document` — SemanticNode（树）+ SemanticRelation（图）、RichText/InlineMark、NodeContent（text/figure/table/equation）；替换原 placeholder
...
2. **确定性生成器** `scripts/generate.py`（`just generate` / `just generate-check`）：从 schema 生成 TS 类型（`packages/typescript/document-model/src/generated/schema.ts`）与 Pydantic 模型（`packages/python/document-model/src/document_model/generated/schema_models.py`）。所有 `$defs` 名称跨 schema 全局唯一，因此两侧均为扁平命名空间。生成物禁止手改，CI 做新鲜度检查。
...
5. **fixtures**（`schemas/fixtures/`，由 `scripts/build_fixtures.py` 确定性生成）：双栏 + 跨栏 Figure + Footnote 的 Physical/Layout 文档、含全部必需 NodeKind 的 Semantic 文档、mock provider Evidence、覆盖 N→1 / 1→N / N→N 三种映射场景的 Mapping。
6. **跨语言 roundtrip 集成测试** `tests/integration/test_cross_language_roundtrip.py`：Python serialize → JSON → TS validate/re-serialize → Python parse，语义一致；TS 侧拒绝几何信息进入 semantic 层。
...
- M1 Exit Gate 五项条件全部满足：核心 schema versioned（`0.1.0`）、roundtrip 稳定、第三方 parser schema 不泄漏（EvidenceBundle `additionalProperties: false` 由测试锁定）、层职责测试完成、mapping many-to-many 验证完成（三种场景 fixture + 测试）。
