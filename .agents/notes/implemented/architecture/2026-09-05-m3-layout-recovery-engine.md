...
4. **ReadingFlowGraph（3.5）**：`pdf_pipeline.reading_flow`。阅读主序由 band/column 结构驱动：band 自上而下、列自左而右；**列内仍按几何 (y, x)**（y 量化到 4pt，避免同行左右颠倒）。不存在文档级全局 `sort(y, x)`。每条 ReadingEdge 带 reason + confidence：SAME_COLUMN/NEXT_COLUMN、BEFORE/AFTER_SPANNING_BLOCK（横跨 band 转换）、CAPTION_ASSOCIATION（layout 关联对的方向覆盖）。
