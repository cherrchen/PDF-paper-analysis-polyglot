...
TeX Live 2026 tlmgr has no standalone `array` package (it ships in `tools`). The CI package list installs `chktex` and `latexindent` instead of `array`. Vale uses a project vocabulary so tool names are not spelling errors. Every `actions/checkout` sets `persist-credentials: false`. Same-repo reusable workflows use the `$/` self-repository syntax.
