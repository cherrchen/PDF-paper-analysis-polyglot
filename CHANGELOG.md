...
- M5 translation and rendering pipeline: translation-layer and render-document schema `0.2.0` (ResourceDocument, RenderTable/Equation/Bibliography blocks, Terminology); structured `TranslationRequest`/`TranslationResult` with mark placeholder protection; `TranslationContext`, terminology discovery/manual override, JSONL translation cache, and OpenAI-compatible HTTP provider; `readable-single-column` RenderProfile/Policy; embedded-image extraction to ResourceStore; unicode→LaTeX equation projection with `\text{}` fallback; LaTeX tabular/thebibliography/`includegraphics` projection; dual `\renderanchor`/`\renderanchorend` hypertargets for multi-fragment anchors.
